...
Section -MainProgram
    ...
    ${If} ${Errors}
        ...
        ${If} $INSTDIR == "$PROGRAMFILES\${APP_NAME}"
            MessageBox MB_OK|MB_ICONEXCLAMATION "Cannot write to $INSTDIR. Would you like to install in your AppData folder instead?" IDYES AppDataInstall
    ...
    ; Copy the main executable files from the dist directory
    File "..\build\dist\PDF2PNG_Converter.exe"
    File "..\build\dist\PDF2PNG_Starter.exe"
    ...
    File /nonfatal "..\bin\poppler-bin\*.*"
    
    SetOutPath "$INSTDIR"
    
    ; Create shortcuts to the starter executable
    CreateDirectory "$SMPROGRAMS\${APP_NAME}"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\${APP_NAME}.lnk" "$INSTDIR\PDF2PNG_Starter.exe" "" "$INSTDIR\icon.ico"
    ...
    ; Add to desktop
    CreateShortCut "$DESKTOP\${APP_NAME}.lnk" "$INSTDIR\PDF2PNG_Starter.exe" "" "$INSTDIR\icon.ico"
