...
poppler_paths = [
    os.path.join(os.path.dirname(os.getcwd()), 'bin', 'poppler-bin'),
    os.path.join(os.path.dirname(os.getcwd()), 'poppler-bin')
]
...
for poppler_bin in poppler_paths:
    if os.path.exists(poppler_bin):
        ...
        break
else:    print("WARNING: No Poppler binaries found. PDF conversion may not work.")

a = Analysis(
    ['../src/splash.py'],
    pathex=[],
    binaries=binaries,
    datas=[('../resources/icon.ico', '.'), ('../resources/banner.txt', '.')],
...
exe = EXE(
    ...
    version='../resources/file_version_info.txt',
)
